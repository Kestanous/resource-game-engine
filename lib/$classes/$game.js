class Game {
  constructor(savedGame) {
    Mixins.buckets(this)
    this.interval = Session.get('interval') || 0.2;
    this.ready = new ReactiveVar(false)
    this.running = new ReactiveVar(false)
    this.currentAge = 'tribal'
  }
  start() {
  ...
  loadAge(age, callback) {
    ...
    for (let assignment in age.assignments) {
      this.addItem(assignment, 'assignments', Assignment, {tasks: age.assignments[assignment]});
    }
    Tracker.afterFlush(() => {
      this.ready.set(true)
      Tracker.autorun( (c) => {
