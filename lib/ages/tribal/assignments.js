    ...
    description: {
      ...
      tasks: {
        ...
        herbalists: {
          ...
          description() {
            return `Herbalists collect usesful plants. Grants ${Game.tickNumber(.02)} hurbs each`
          }, 
          prerequisites() { 
            let medicine = this.state.unlockedBuckets('technology', 'medicine')
            if (medicine) {
              return medicine.owned() 
            }
          }
        },
        archaeologists: {
...
import {Assignment} from 'meteor/private:game-engine';
