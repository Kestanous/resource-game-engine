const actions = {
  ...
  investigate: {
    ...
    prerequisites() {
      return this.state.buckets('resources', 'scrap').getValue() > 49
    }
  },
  "hunt": {
    description: {
  ...
  butcher: {
    ...
    prerequisites() {
      let tech = this.state.buckets('technology', 'animal farm')
      if (tech) return tech.owned()
    }
  },
  'build shelter': {
    ...
    prerequisites() {
      let tech = this.state.buckets('technology', 'crafting')
      if (tech) return tech.owned()
    }
  },
  'advance age!': {
    ...
    prerequisites() {
      let shelter = this.state.buckets('resources', 'shelter')
      , food = this.state.buckets('resources', 'food')
      , animalFarm = this.state.buckets('technology', 'animal farm');
...
import {Action} from 'meteor/private:game-engine';
