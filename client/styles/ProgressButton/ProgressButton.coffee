...
Template.ProgressButton.onCreated ->
  @validate = =>
    # we will consider the fill/horizontal as default
    if @button.getAttribute('data-style') == null
      @button.setAttribute 'data-style', 'fill'
  ...
  @setProgress = (val) => @progress.style[@progressProp] = 100 * val + '%'
  @stop = (status) =>
    self = this
    setTimeout (->
      # fade out progress bar
      ...
      onEndTransFn = (ev) ->
        if support.transitions and ev.propertyName != 'opacity'
          return
        @removeEventListener transEndEventName, onEndTransFn
        self.$(self.progress).addClass 'notransition'
        self.progress.style[self.progressProp] = '0%'
        self.progress.style.opacity = 1
        return
      ...
      if typeof status == 'number'
        ...
        # after options.statusTime remove status
        Meteor.setTimeout (->
          self.$(self.button).removeClass statusClass
          self.enable()
          return
        ), self.options.statusTime
      else
        self.enable()
      # remove class state-loading from the button
      self.$(self.button).removeClass 'state-loading'
      return
    ), 100

Template.ProgressButton.onRendered ->
  @button = @find('.progress-button')
  @validate()
  @create()
  @options = statusTime: 1500
  ...
  @data.duration ?= 20
  # new ProgressButton @find('.progress-button'), callback: (instance) =>
  #   progress = 0
  #   unitOfProgress = 0.01
...
Template.ProgressButton.events
  'click': ->
    self = Template.instance()
    # disable the button
    self.button.setAttribute 'disabled', ''
    # add class state-loading to the button (applies a specific transform to the button depending which data-style is defined - defined in the stylesheets)
    self.$(self.progress).removeClass 'notransition'
    self.$(self.button).addClass 'state-loading'
    setTimeout (->
      if typeof self.options.callback == 'function'
        self.options.callback self
      else
